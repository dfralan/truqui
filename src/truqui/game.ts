...
const SUITS: Record<string, { name: string; img: string; color: string }> = {
...
const TRUCO_POINTS = [1, 2, 3, 4] as const;
export const TRUCO_NAMES = ['', 'Truco', 'Retruco', 'Vale 4'] as const;

export const ENVIDO_NAMES: Record<EnvidoLevel, string> = {
  2: 'Envido', 3: 'Real Envido', falta: 'Falta Envido',
};

export const ENVIDO_ACCEPT_PTS: Record<2 | 3, number> = { 2: 2, 3: 3 };
export const ENVIDO_DECLINE_PTS: Record<EnvidoLevel, number> = { 2: 1, 3: 2, falta: 1 };
...
function cardPower(id: CardId): number {
  return TRUCO_POWER[id] ?? 0;
}

function envidoCardValue(rank: number): number {
...
export function envidoPoints(cards: CardId[]): number {
  ...
  let best = 0;
  for (const id of cards) best = Math.max(best, envidoCardValue(parseCard(id).rank));
  for (let i = 0; i < cards.length; i++) {
    for (let j = i + 1; j < cards.length; j++) {
      const a = parseCard(cards[i]!);
      const b = parseCard(cards[j]!);
      if (a.suit === b.suit) best = Math.max(best, envidoCardValue(a.rank) + envidoCardValue(b.rank));
    }
  }
  return best;
}

export function allHandCards(hand: HandState, player: PlayerSlot | number): CardId[] {
  const p = slotIndex(player);
  return [...hand.hands[p], ...hand.playedCards[p]];
}

export function faltaEnvidoPoints(scores: [number, number]): number {
  return 30 - Math.max(scores[0], scores[1]);
}

function envidoWinner(hand: HandState): PlayerSlot {
...
export function canCallEnvido(hand: HandState): boolean {
...
function trickWinner(p0Card: CardId, p1Card: CardId): PlayerSlot | null {
...
export function slotIndex(n: number): PlayerSlot {
  return Number(n) === 0 ? 0 : 1;
}

export function rivalIndex(n: number): PlayerSlot {
  return slotIndex(n) === 0 ? 1 : 0;
}

export function normalizeHand(hand: HandState): HandState {
  const h = structuredClone(hand) as HandState & Record<string, unknown>;
  const raw = hand as HandState & { hands?: Record<string, CardId[]> };
  h.hands = {
    0: [...(raw.hands?.[0] ?? raw.hands?.['0'] ?? [])],
    1: [...(raw.hands?.[1] ?? raw.hands?.['1'] ?? [])],
  };
  h.trickCards = {
    0: raw.trickCards?.[0] ?? raw.trickCards?.['0' as unknown as 0] ?? null,
    1: raw.trickCards?.[1] ?? raw.trickCards?.['1' as unknown as 1] ?? null,
  };
  h.playedCards = {
    0: [...(raw.playedCards?.[0] ?? raw.playedCards?.['0' as unknown as 0] ?? [])],
    1: [...(raw.playedCards?.[1] ?? raw.playedCards?.['1' as unknown as 1] ?? [])],
  };
...
export function dealNewHand(previousMano: PlayerSlot | null = null): HandState {
  const mano: PlayerSlot = previousMano == null ? 0 : rivalIndex(previousMano);
...
export function newGameState(existingScores: [number, number] = [0, 0]): RoomState {
  return { status: 'playing', players: {}, scores: existingScores, hand: dealNewHand(), updatedAt: Date.now() };
}

export function applyPlay(hand: HandState, player: number, cardId: CardId): HandState {
  const h = normalizeHand(hand);
  const p = slotIndex(player);
  if (h.phase !== 'playing' || h.envidoPending || h.turn !== p || !h.hands[p].includes(cardId)) return h;
...
export function applyTrucoCall(hand: HandState, caller: number): HandState {
  const h = normalizeHand(hand);
  const c = slotIndex(caller);
  if (h.phase !== 'playing' || h.trucoPending || h.envidoPending || h.truco >= 3) return h;
  const next = structuredClone(h);
...
export function applyTrucoResponse(hand: HandState, accept: boolean): HandState {
  const h = normalizeHand(hand);
...
export function applyEnvidoCall(hand: HandState, caller: number, level: EnvidoLevel = 2): HandState {
  const h = normalizeHand(hand);
  const c = slotIndex(caller);
...
export function applyEnvidoResponse(
  hand: HandState,
  scores: [number, number],
  responder: number,
  action: EnvidoAction,
): { hand: HandState; scores: [number, number] } {
  const h = normalizeHand(hand);
  const r = slotIndex(responder);
  if (!h.envidoPending || h.envidoWaitingOn !== r) return { hand: h, scores: [...scores] };
  const next = structuredClone(h);
...
export function finishHand(state: RoomState): RoomState {
  const next = structuredClone(state);
  if (!next.hand) return next;
  const pts = next.hand.handPoints;
  if (next.hand.winner !== null) next.scores[next.hand.winner] += pts;
...
function handWinner(trickWinners: (PlayerSlot | null)[]): PlayerSlot {
