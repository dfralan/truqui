...
export async function decrypt(keyB64, ciphertextB64) {
    const raw = new Uint8Array(b64Decode(keyB64));
...
function bech32Encode(prefix, data) {
    const convertBits = (values, from, to) => {
        let acc = 0;
        let bits = 0;
        const ret = [];
