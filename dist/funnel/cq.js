...
function normalizeForHash(obj, parentKey) {
    if (obj === null || typeof obj === 'string' || typeof obj === 'boolean')
        return obj;
    if (typeof obj === 'number') {
        if (!Number.isInteger(obj))
            throw new Error('CQ: floats no permitidos');
        return obj;
    }
    ...
    if (!isPlainObject(obj))
        throw new Error('CQ: tipo no soportado');
    const keys = Object.keys(obj).sort();
    const out = {};
    for (const k of keys) {
        if (k === 'id' || k === 'signatures')
            continue;
...
export async function computeId(obj) {
...
export async function finalize(partial) {
